...
class KonapApp(object):
    ...
    def plugin_load(self, plugin_dirs, cls):
        ...
        @plugin_dirs - list of string, each string
            represents directory with plugins
            ...
            for root_path, directory, file_name in os.walk(directory):
                for name in file_name:
                    if name.endswith('.py') and not name.startswith('__'):
                        ...
                        finded_plugin = find_subclass(name, plugin_path, cls)
                        if finded_plugin:
...
def find_subclass(plugin_name, plugin_path, cls):
    """
    Function for seraching given class in
    """
    imported_module = imp.load_source(plugin_name, plugin_path)
...
def plugin_load(plugin_dirs, cls):
    """
    Function for loading converting plugins
    @plugin_dirs - list of string, each string represents
...
def plugin_instance(plugin_list):
    """
    Function converting classes of load plugin in
    plugin instancecs
    Return list of instances
